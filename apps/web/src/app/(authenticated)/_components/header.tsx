...
export function AppHeader({ session }: Props) {
  ...
  return (
    <>
      ...
      >
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2" style={{ fontSize: 12, fontWeight: 500 }}>
          {crumbs.map((crumb, i) => (
            <span key={i} className="flex items-center gap-2">
              {i > 0 && (
                <span className="material-symbols-outlined" style={{ fontSize: 14, color: 'rgba(229,226,225,0.2)' }}>
                  chevron_right
                </span>
              )}
              <span style={{
                color: i === crumbs.length - 1 ? 'var(--v2-accent)' : 'rgba(229,226,225,0.4)',
        ...
        <div className="flex-1 max-w-sm mx-12">
          ...
          >
            ...
            <span className="ml-auto flex items-center gap-1">
              <kbd style={{ background: 'rgba(75,68,85,0.4)', padding: '1px 5px', borderRadius: 3, fontSize: 9, fontFamily: 'monospace' }}>\u2318</kbd>
              <kbd style={{ background: 'rgba(75,68,85,0.4)', padding: '1px 5px', borderRadius: 3, fontSize: 9, fontFamily: 'monospace' }}>K</kbd>
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, var(--v2-accent), var(--v2-accent-dim))', border: '1px solid rgba(var(--v2-accent-rgb), 0.3)' }}
          >
    ...
    <div className="flex items-center gap-2">
      <div
        ...
        style={{
          ...
          boxShadow: isConnected ? '0 0 8px rgba(35,222,203,0.5)' : '0 0 8px rgba(255,180,171,0.5)',
        }}
      />
      <span style={{ color: 'rgba(229,226,225,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
